...
class Shapla_Instagram_Widget extends WP_Widget{
	...
	function shapla_instagram_widget() {
		$widget_ops = array( 'classname' => 'shapla-instagram', 'description' => __( 'A widget that displays your Instagram feed, posts, or likes.', 'shapla' ) );
		$control_ops = array( 'width' => 200, 'height' => 350, 'id_base' => 'shapla-instagram' );
		$this->WP_Widget( 'shapla-instagram', __( 'Shapla Instagram Photos', 'shapla' ), $widget_ops, $control_ops );
	}
	...
	function form( $instance ){
		...
		<p>
			<label for="<?php echo $this->get_field_id('username'); ?>"><?php _e( 'Username:', 'shapla' ); ?></label>
			<input type="text" class="widefat" id="<?php echo $this->get_field_id('username'); ?>" name="<?php echo $this->get_field_name('username'); ?>" value="<?php echo $instance['username']; ?>">
		</p>

		<p>
			<label for="<?php echo $this->get_field_id('count'); ?>"><?php _e( 'Number of Photos:', 'shapla' ); ?></label>
		...
		<p>
			...
			<select id="<?php echo $this->get_field_id('size'); ?>" name="<?php echo $this->get_field_name('size'); ?>" class="widefat">
				...
				<option value="large" <?php selected('large', $instance['size']) ?>><?php _e('Large', 'shapla'); ?></option>
			</select>
		</p>

		<p>
			<label for="<?php echo $this->get_field_id('cachetime'); ?>"><?php _e( 'Cache every:', 'shapla' ); ?></label>
	...
	function scrape_instagram( $username, $slice = 9, $cachetime = 5 ) {
		if ( false === ( $instagram = get_transient( 'shapla-instagram-photos-' . sanitize_title_with_dashes($username) ) ) ) {
			$remote = wp_remote_get( 'http://instagram.com/' . trim($username) );

			if (is_wp_error($remote)) {
	  			return new WP_Error( 'site_down', __('Unable to communicate with Instagram.', 'shapla') );
			}

			if ( 200 != wp_remote_retrieve_response_code( $remote ) ) {
  				return new WP_Error( 'invalid_response', __('Instagram did not return a 200.', 'shapla') );
			}
			...
			if (!$insta_array) {
	  			return new WP_Error( 'bad_json', __('Instagram has returned invalid data.', 'shapla') );
			}
			...
			set_transient('shapla-instagram-photos-'.sanitize_title_with_dashes($username), $instagram, apply_filters('null_instagram_cache_time', HOUR_IN_SECONDS * $cachetime ) );
		}
